            ...
            SafeArea(
              child: NavigationRail(
                ...
                destinations: [
                  ...
                  NavigationRailDestination(
                      icon: Icon(Icons.people), label: Text('MyPage'))
                ],
        ...
        for (var pair in appState.favorites)
          ListTile(
            ...
            leading: IconButton(
              ...
              onPressed: () {
                // 즐겨찾기 해제
                appState.removeFavaorite(pair);

                // 스낵바 메시지
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text("삭제됨: ${pair.asLowerCase}"),
                      ...
                      SnackBar(
                        content: Text(
                          appState.favorites.contains(pair)
                              ? "추가됨: ${pair.asLowerCase}"
                              : "삭제됨: ${pair.asLowerCase}",
                        ),
                        duration: Duration(seconds: 1),
                      ),
                    );
...
class MyPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [Image.asset('images/profile.jpeg'), Text("My Page")],
      ),
    );
  }
}
